...
{
    GLextension extension = static_cast<GLextension>(glbinding::Meta::getExtension(extensionName));
...
std::set<GLextension> ContextInfoES::extensions(std::set<std::string> * unknown)
{
...
std::string ContextInfoES::renderer()
{
    return std::string(reinterpret_cast<const char *>(glGetString(GL_RENDERER)));
}

std::string ContextInfoES::vendor()
{
    return std::string(reinterpret_cast<const char *>(glGetString(GL_VENDOR)));
}

Version ContextInfoES::version()
{
    Version version;
